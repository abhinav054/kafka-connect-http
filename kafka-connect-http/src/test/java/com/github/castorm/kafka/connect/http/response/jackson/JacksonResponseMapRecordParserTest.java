...
import static com.github.castorm.kafka.connect.http.response.jackson.JacksonResponseMapRecordParserTest.Fixture.*;
...
public class JacksonResponseMapRecordParserTest {


    JacksonResponseMapRecordParser parser;
    ...
    @BeforeEach
    void SetUp(){
        parser = new JacksonResponseMapRecordParser(__ -> config,recordParser,serializer);
    ...
    void givenRecords_whenGetRecords_thenAllReturned() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).collect(toList())).hasSize(2);
    }
    ...
    void givenRecordWithKey_whenGetRecords_thenItemKeyMapped() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getKey()).isEqualTo("value");
    }
    ...
    void givenRecordWithoutKey_whenGetRecords_thenItemKeyNull() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getKey()).isNull();
    }
    ...
    void givenRecordWithTimestamp_whenGetRecords_thenItemTimestampMapped() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getTimestamp()).isEqualTo("value");
    }
    ...
    void givenRecordWithoutTimestamp_whenGetRecords_thenItemTimestampNull() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getTimestamp()).isNull();
    }
    ...
    void givenRecordWithOffset_whenGetRecords_thenItemOffsetMapped() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getOffset()).isEqualTo(ImmutableMap.of("k", "v"));
    }
    ...
    void givenRecordWithValue_whenGetRecords_thenItemBodyMapped() {
        ...
        assertThat(parser.getRecords(itemObject.getBytes()).findFirst().get().getBody()).isEqualTo("value");
    }
    ...
    private void givenRecords(JsonNode... records) {
        ...
        given(serializer.deserialize(itemObject.getBytes())).willReturn(itemObjectJson);
        given(serializer.getArrayAt(itemObjectJson, pointer)).willReturn(Stream.of(records));
    ...
    interface Fixture {
        ...
        JsonNode itemObjectJson = deserialize(itemObject);
